...
export default function ClickDropdown({ text, items }: {
    text: string,
    ...
    useEffect(() => {
        ...
        const callback = (e: MouseEvent) => {
            if (!itemsDiv.current!.contains(e.target as HTMLElement)
    ...
    return <div className="dropdown-click">
        <div 
        ...
        <div className="dropdown-click-items-container">
            <div 
                className={"dropdown-click-items" + (isShow ? "" : " dropdown-click-items-hide")}
                style={{
...
const ArrowDown = ({ isInverted }: {
    isInverted: boolean,
}): JSX.Element => {
    return <svg 
        className={"dropdown-click-arrow" + (isInverted ? " dropdown-click-arrow-inverted" : "")} 
        height="10" 
        width="18"
    >
