...
export default function UltimateTictactoe(): JSX.Element {
  return (
    <div className="ultimate-tictactoe">
...
const toBytecoderString: ToBytecoderString =
  bytecoder.toBytecoderString as unknown as ToBytecoderString;
...
const Terminal = (): JSX.Element => {
  ...
  const updateDialog = useCallback(() => {
    ...
    setDialog((dialog) => [...dialog, response]);
    scrollToBottom();
  }, [scrollToBottom]);

  const startup = useCallback(() => {
    const response = agent.getResponse(
      toBytecoderString('startup')
    ).nativeObject;
    setDialog([response]);
  }, []);
  ...
  const getResponse = useCallback(() => {
    const response = agent.getResponse(
      toBytecoderString(inputValue)
    ).nativeObject;
    setDialog((dialog) => [...dialog, `> ${inputValue}`, response]);
    setInputValue('');
    scrollToBottom();
  ...
  return (
    <div className="ultimate-tictactoe-terminal">
      <div className="ultimate-tictactoe-terminal-log" ref={logRef}>
        {dialog.map((text, i) => (
          <div className="ultimate-tictactoe-terminal-text" key={i}>
            {text}
          </div>
        ))}
      </div>
