    ...
    details: [
      {
        ...
        detail:
          'Lead workshops on Godot game development engine for new members in Academic Year 2023-2024, equipping members with game development skills within 4 weeks.',
      },
      {
        image: dreamyImage,
        detail:
          'Collaborated with group members to produce a platformer game (Guardian of the Dreamy World) using Godot engine within a less than half a year, available on itch.io.',
      },
      {
        image: overchargedImage,
        detail:
          'Cooperated with club member to develop bullet-hell game (Overcharged Magic) within one week using Godot engine, available on itch.io.',
      },
    ],
  },
  {
    ...
    details: [
      {
        ...
        detail:
          'Spearheaded training activities for club members as captain in Academic Year 2023-2024.',
      },
      {
        image: spSpeedImage,
        detail:
          'Coordinated with members to win second place for team category and sixth place for individual category in SP Speed Chess Competition (2023).',
      },
      {
        image: spBowImage,
        detail:
          'Coordinated with group members to win first place in Singapore Poly Chinese Chess Bow (2022) and third place in NUS Invitational Interschool Competition (2023)',
      },
    ],
  },
];

const dropdownItems = ['Game Development', 'Chinese Chess'];
...
const ActivityElaboration = ({
  ...
  const highlightHandler = useCallback<(x: number) => () => void>(
    (index) => {
      return () => {
        switch (index) {
          case (highlightIndex - 1 + activity.details.length) %
            ...
            setHighlightIndex(
              (state) =>
                (state - 1 + activity.details.length) % activity.details.length
            );
  ...
  return (
    <div
      className={
        'activities-activity' + (isShow ? '' : ' activities-activity-hide')
      }
      ref={root}
    >
      ...
      <div className="activities-activity-body">
        {activity.details.map((detail, detailIndex) => (
          <ActivityDetailElaboration
            ...
            position={
              detailIndex ===
              (highlightIndex - 1 + activity.details.length) %
                activity.details.length
                ? 'top'
...
const ActivityDetailElaboration = ({
  detail,
  position,
  setHighlight,
  isGoingDown,
  ...
  const isTransitionDisabled =
    (position === 'top' && !isGoingDown) ||
    (position === 'bottom' && isGoingDown);
    ...
    <div
      className={
        ...
        (!isDesktop
          ...
          : '') +
        (!isDesktop && isTransitionDisabled
          ? ' activities-activity-detail-transition-disabled'
          : '')
      }
      ref={root}
    >
      ...
      <div className="activities-activity-text" onClick={setHighlight}>
        {detail.detail}
      </div>
    </div>
  );
