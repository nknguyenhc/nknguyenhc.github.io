...
const Description = (): JSX.Element => {
  return (
    <div className="css-description">
      When transmitting data, how do you know if the trasmitted data is
      corrupted? The idea is, instead of transmitting raw data, we transmitted
      encoded data, so that upon receival, in the case that there are slight
      errors, the data can still be decoded to give the correct raw data. We
      explore 3 different schemes of coding:
...
const SourceCode = (): JSX.Element => {
  const codes = useMemo<Array<string>>(
    () => ['hamming_code.cpp', 'convolutional.cpp', 'RC_929.cpp'],
    []
  );
  ...
  return (
    <div className="css-source">
      <div className="css-source-description">
        <div>
          We programmed the code in C++, which is the backbone of our APIs. The
          code is responsible for encoding and decoding each type of coding
          scheme.
        </div>
        <div>
          Our Nodejs backend interacts with the cpp code through stdin and
          stdout.
        </div>
        <div>
          Our frontend makes use of the APIs for encoding and decoding. For
          stepwise calculation, we coded the coding scheme logic to frontend.
        </div>
        <div>Below is our source code in C++:</div>
      </div>
...
const useCssCodeContent = (filename: string) => {
  ...
  useEffect(() => {
    fetch(
      `https://raw.githubusercontent.com/tranvietkhoa/Coding-Schemes-Simulation/main/${filename}`
    )
      .then((res) => res.text())
...
const CodeContent = ({ name }: { name: string }): JSX.Element => {
  ...
  const foldGutterExtension = useMemo(
    () =>
      foldGutter({
        markerDOM: (isOpen) => {
  ...
  return (
    <div className="css-source-code">
      ...
      <CodeMirror
        ...
        readOnly
        className={
          'css-source-code-content' +
          (isShow ? '' : ' css-source-code-content-hidden')
        }
        ref={codeRef}
        style={{
