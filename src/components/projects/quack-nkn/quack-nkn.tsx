...
const techstacks: Array<TechStackType> = [
  {
    name: 'Puppeteer',
    description:
      'Puppeteer is capable of launching a Chrome browser in headless mode (i.e. without a browser actually being launched). With its knowledge of the internals of chrome, it can be easily used to navigate through a page and take a screenshot.',
    link: 'https://pptr.dev/',
    icon: puppeteerIcon,
  },
  {
    name: 'Telegram',
    description:
      'Telegram provides its bot API and the accompanying elaborate documentation over its npm package node-telegram-bot-api.',
  ...
  },
  {
    name: 'Typescript',
    description:
      "Typed version of Javascript (Nodejs). Arguably, Python is a more popular choice of language for Telegram bot. I chose Typescript due to its type hint and quick access to node modules' documentation. Furthermore, Puppeteer is only available on Nodejs.",
...
const Readme = (): JSX.Element => (
  <div className="quack-nkn-readme">
    <div>
      Have you ever wanted to track a particular website? Especially those with
      internships? Personally, I always wanted to apply to those internship
      positions as soon as they are open. But too lazy to open them
      everyday/week? That's why I developed this bot!
    </div>
    <div className="quack-nkn-readme-techstacks">
      ...
      <div className="quack-nkn-readme-techstacks-list">
        {techstacks.map((techstack) => (
          <div
            className="quack-nkn-readme-techstacks-list-item"
            key={techstack.name}
          >
            <a
...
const FeedbackForm = (): JSX.Element => (
  <iframe
    src="https://quack-nkn.nknguyenhc.net/feedback"
    className="quack-nkn-feedback"
    title="Quack Nkn Feedback"
  />
);
...
const Features = (): JSX.Element => {
  ...
  const staticPosition = useMemo(
    () =>
      scrollPosition -
      (staticRef.current ? staticRef.current.getBoundingClientRect().top : 1) -
      window.scrollY,
    [scrollPosition]
  );
  ...
  const dynamicPosition = useMemo(
    () =>
      isDesktop
        ? scrollPosition -
          (dynamicRef.current
            ? dynamicRef.current.getBoundingClientRect().top
            : 1) -
          window.scrollY
        : -(dynamicRef.current
            ? dynamicRef.current.getBoundingClientRect().top
          ...
          dynamicHeight +
          windowHeight,
    [isDesktop, scrollPosition, dynamicHeight, windowHeight]
  );
  ...
  const frames = useMemo<Array<string>>(() => {
    const toFrameNumber = (i: number): string =>
      '0'.repeat(2 - Math.floor(Math.log(i) / Math.log(10))) + i;
    return Array.from(Array(frameCount).keys()).map(
      (i) =>
        `${process.env.PUBLIC_URL}/videos/quack-nkn-tracker/frame-${toFrameNumber(i + 1)}.jpg`
    );
  }, [frameCount]);
  ...
  return (
    <div className="quack-nkn-features">
      <div
        ref={staticRef}
        ...
        <div
          ...
          style={{
            position:
              (isDesktop
                ? staticPosition > 0
                : staticPosition > staticHeight - windowHeight &&
                  staticPosition !== -1) &&
        ...
        >
          <div className="quack-nkn-features-text">
            <div
              ...
              style={{
                transform: `translateX(${
                  staticPosition <= 0 ||
                  staticPosition > techstackScrollBreakpoints.end
                    ? '0px'
                    : staticPosition > 0 &&
                        staticPosition <= techstackScrollBreakpoints.mid
            ...
            <div className="quack-nkn-features-description">
              <div className="quack-nkn-features-description-title">
                Reminders
              </div>
              <div className="quack-nkn-features-description-details">
                <div>
                  Add your reminders, and the bot will deliver the reminders to
                  you on your requested times.
                </div>
                <ul>
                  <li>
                    <code>/add</code>
                  ...
                  <li>
                    <code>&lt;your reminder&gt;</code>
                  </li>
                  <li>
                    <code>&lt;your frequency&gt;</code>
                  </li>
                </ul>
              </div>
            </div>
      ...
      >
        <div
          ...
          style={{
            position:
              dynamicPosition > 0 && dynamicPosition < scrollBreakpoints.end
                ? 'fixed'
            ...
            bottom:
              !isDesktop || dynamicPosition >= scrollBreakpoints.end ? 0 : '',
          }}
        >
          <div className="quack-nkn-features-text">
            <div
              ...
              style={{
                transform: `translateX(${
                  dynamicPosition <= 0 ||
                  dynamicPosition > techstackScrollBreakpoints.end
                    ? '0px'
                    : dynamicPosition > 0 &&
                        dynamicPosition <= techstackScrollBreakpoints.mid
            ...
            <div className="quack-nkn-features-description">
              ...
              <div className="quack-nkn-features-description-details">
                <div>
                  Add the website you want to track, and the bot will send you
                  screenshots on your requested times.
                </div>
                <ul>
                  <li>
                    <code>/add</code>
                  ...
                  <li>
                    <code>&lt;your website URL&gt;</code>
                  </li>
                  <li>
                    <code>&lt;your tracking details&gt;</code>
                  </li>
                  <li>
                    <code>&lt;your time & frequency&gt;</code>
                  </li>
                </ul>
              </div>
            </div>
          ...
          <div
            ...
            style={{
              ...
              backgroundColor:
                dynamicPosition > scrollBreakpoints.start &&
                dynamicPosition <= scrollBreakpoints.morph
                  ? `rgba(0, 0, 0, ${(dynamicPosition - scrollBreakpoints.start) / (scrollBreakpoints.translate - scrollBreakpoints.start)})`
                  : dynamicPosition > scrollBreakpoints.morph &&
                      dynamicPosition <= scrollBreakpoints.fade
                    ? 'rgb(0, 0, 0)'
                    : dynamicPosition > scrollBreakpoints.fade &&
                        dynamicPosition <= scrollBreakpoints.end
            ...
            {frames.map((frame, i) => (
              <img
                ...
                style={{
                  ...
                  right: isDesktop
                    ? dynamicPosition > scrollBreakpoints.translate &&
                      dynamicPosition <= scrollBreakpoints.morph
                      ? ((dynamicPosition - scrollBreakpoints.translate) /
                          (scrollBreakpoints.morph -
                            scrollBreakpoints.translate)) *
                          translateDistance +
                        padding
                      : dynamicPosition <= scrollBreakpoints.translate ||
                          dynamicPosition > scrollBreakpoints.end
                        ...
                        : dynamicPosition > scrollBreakpoints.fade &&
                            dynamicPosition <= scrollBreakpoints.end
                          ? ((scrollBreakpoints.end - dynamicPosition) /
                              (scrollBreakpoints.end -
                                scrollBreakpoints.fade)) *
                              translateDistance +
                            padding
                          : translateDistance + padding
                    : 0,
                  zIndex:
                    (dynamicPosition <= scrollBreakpoints.morph && i === 1) ||
                    (dynamicPosition > scrollBreakpoints.fade &&
                      i === frameCount - 1)
                      ? 2
                      : (dynamicPosition - scrollBreakpoints.morph) /
                            (scrollBreakpoints.fade - scrollBreakpoints.morph) >
                            i / frameCount &&
                          (dynamicPosition - scrollBreakpoints.morph) /
                            (scrollBreakpoints.fade - scrollBreakpoints.morph) <
                            (i + 1) / frameCount
