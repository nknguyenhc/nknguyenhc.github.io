...
export default function DjangoHerokuGuide(): JSX.Element {
  ...
  const displayOffset = useMemo<number>(
    () => (isDesktop ? 100 : 60),
    [isDesktop]
  );
  ...
  const setSectionStyle = useCallback<
    (sectionIndex: number) => (top: number, bottom: number) => CSSProperties
  >(
    (sectionIndex) => (top, bottom) => {
      if (top > bodyGap * partial) {
        ...
        bottom <= window.innerHeight - displayOffset &&
        bottom > window.innerHeight - bodyGap * partial
      ) {
        return {
          opacity:
            (bottom - (window.innerHeight - bodyGap * partial)) /
          ...
          bottom:
            bottom - top > window.innerHeight
              ? displayOffset +
                (window.innerHeight - displayOffset - bottom) * scrollLag
              : '',
          top:
            bottom - top <= window.innerHeight
              ? displayOffset + (top - displayOffset) * scrollLag
              : '',
          width: bodyWidth,
        };
      } else {
  ...
  return (
    <div className="central-body" ref={root}>
      ...
      <img
        className="django-heroku-banner"
        src={data.banner}
        alt="guide banner"
      />
...
const SectionDiv = ({
  ...
  const resetDimensions = useCallback<() => void>(() => {
    ...
    const containerRect: DOMRect =
      containerDiv.current!.getBoundingClientRect();
        ...
        <div className="django-heroku-section-body">
          {section.body.map((subsection, subsectionIndex) =>
            subsection instanceof Paragraph ? (
              ...
              <div
                className="django-heroku-section-body-img"
                key={subsectionIndex}
              >
