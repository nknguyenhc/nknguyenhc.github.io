...
const Stepper = ({
  ...
  useEffect(() => {
    const callback = () => {
      ...
      const elements: Array<HTMLElement> = steps.map(
        (step) => document.querySelector(`#${step.id}`)!
      );
  ...
  useEffect(() => {
    const callback = () =>
      setIsOverflow(
        rootDiv.current!.scrollWidth > rootDiv.current!.clientWidth
      );
    callback();
  ...
  return (
    <div
      className={
        'stepper' +
        (isOverflow ? ' stepper-overflow' : '') +
        (isScrolled && showWords ? ' stepper-scrolled' : '')
      }
      ref={rootDiv}
      onScroll={() => {
...
const StepperStepDiv = ({
  ...
  const handleClickOnParent = useCallback(
    (event: MouseEvent): void => {
      if (
        ...
        isActive ||
        !isDesktop ||
        (event.target as HTMLElement).contains(circle.current)
      ) {
  ...
  return (
    ...
    >
      ...
      <div
        className={
          'stepper-step-text' + (showWords ? '' : ' stepper-step-text-hide')
        }
      >
        {text}
      </div>
